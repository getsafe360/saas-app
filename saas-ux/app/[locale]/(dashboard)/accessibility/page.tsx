  ...
  issues: [
    {
      ...
      elements: 1,
      details:
        "Dialog elements require accessible names so screen readers can identify them.",
    },
    {
      severity: "critical",
      summary:
...
function calculateAuditCost(input = 0, output = 0, level = "all") {
  const INPUT_TOKEN_PRICE = 0.000001;
  const OUTPUT_TOKEN_PRICE = 0.000003;
...
export default function AccessibilityResultPage() {
  ...
  const costCritical = calculateAuditCost(
    inputTokens,
    outputTokens,
    "critical"
  );
        ...
        <div className="mb-8">
          ...
          <ul className="flex flex-wrap gap-2">
            {passedChecks.map((check, i) => (
              <li
                key={i}
                className="flex items-center gap-1 border border-green-400 text-green-200 px-3 py-1 rounded-full text-xs font-semibold bg-[#142918]"
              >
          ...
          {criticalIssues.length === 0 ? (
            ...
            <ol className="space-y-4 pl-4 list-decimal">
              {criticalIssues.map((issue, i) => (
                <li
                  key={i}
                  className="bg-red-900/50 border-l-4 border-red-500 p-3 rounded"
                >
        ...
        <div className="flex flex-col md:flex-row gap-4 mt-8">
          ...
          >
            {processing === "critical" ? (
              ...
              <>
                {t("fix_critical_btn")}{" "}
                <span className="ml-2">{costCritical.TotalCost}</span>
              </>
            )}
          </button>
          <button
          ...
          >
            {processing === "all" ? (
              ...
              <>
                {t("supreme_repair_btn")}{" "}
                <span className="ml-2">{costAll.TotalCost}</span>
              </>
            )}
          </button>
        </div>
