...
type PackPriceRow = typeof appPackPrices.$inferSelect;
...
async function taxBehaviorForRegion(region: string | null) {
  ...
  const [p] = await db
    ...
    .where(eq(taxDisplayPolicies.country, region))
    .limit(1);
  if (!p) return 'exclusive';
  return p.taxLabel === 'vat_included' ? ('inclusive' as const) : ('exclusive' as const);
}
...
async function ensureProduct(opts: { existingId?: string | null; name: string; slug: string; kind: 'plan' | 'pack' }) {
  if (opts.existingId) {
    try {
      return await stripe.products.retrieve(opts.existingId);
    } catch {
...
export async function syncStripeForPlans() {
  ...
  for (const pl of plansList) {
    const rows: PlanPriceRow[] = await db.select().from(planPrices).where(eq(planPrices.planId, pl.id));
    ...
    await Promise.all(
      rows
        ...
        .map((r) =>
          db.update(planPrices).set({ stripeProductId: product.id }).where(eq(planPrices.id, r.id))
        )
    );
...
export async function syncStripeForPacks() {
  ...
  for (const pk of packs) {
    const rows: PackPriceRow[] = await db.select().from(appPackPrices).where(eq(appPackPrices.packId, pk.id));
    if (rows.length === 0) continue;
    ...
    await Promise.all(
      rows
        .filter((r) => r.stripeProductId !== product.id)
        .map((r) =>
          db.update(appPackPrices).set({ stripeProductId: product.id }).where(eq(appPackPrices.id, r.id))
        )
    );
